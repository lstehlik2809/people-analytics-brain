...
2. The authors’ summary seems to turn the Titanic’s story of chivalry on its head. But what does the observed female survival gap actually mean when sex appears to bundle together social treatment and physical capacity? Fortunately, the authors made their `r xfun::embed_file("data/sd01.xlsx", text = "data publicly available")`, so I did not have to stop at asking these questions - I could try to answer them.

But before getting to that, let’s reproduce the paper’s original numbers and make sure we are playing on the same field.
